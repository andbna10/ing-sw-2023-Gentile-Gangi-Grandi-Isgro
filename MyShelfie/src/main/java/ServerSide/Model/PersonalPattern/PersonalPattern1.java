...
public class PersonalPattern1 extends PersonalGoalCard {
    ...
    public int validated(Bookshelf bookshelf) {
        ...
        if(bookshelf.getTile(1,4) != null){
            if(bookshelf.getTile(1, 4).getType() == ItemType.PLANTS) count++;
        }
        if(bookshelf.getTile(2,3) != null){
            if(bookshelf.getTile(2, 3).getType() == ItemType.PLANTS) count++;
        }
        if(bookshelf.getTile(3,1) != null){
            if(bookshelf.getTile(3, 1).getType() == ItemType.PLANTS) count++;
        }
        if(bookshelf.getTile(5,2) != null){
            if(bookshelf.getTile(5, 2).getType() == ItemType.PLANTS) count++;
        }
